...
uint8_t Utils::MatrixMultiplication(int row, std::array< uint8_t, Block::BLOCK_DIMENSION> column) {

    uint8_t result = 0;
    for(int i = 0; i < Block::BLOCK_DIMENSION; i+=1) {
        uint8_t tmp = Utils::specialMultiplication(column[i], Utils::matrix[row][i]);
        result ^= tmp;
    }
