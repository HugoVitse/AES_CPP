...
const std::array<std::array<u_int8_t, 16>, 16> Key::Sbox = {{
...
void Key::SubWord(std::array<uint8_t, Key::WORD_SIZE>* word) {
    for(int i=0; i < word->size(); i+=1){
        (*word)[i] = SBoxSubstitution(word->at(i));
    }
}

uint8_t Key::SBoxSubstitution(uint8_t byte) {
    uint8_t high = byte >> 4;
    uint8_t low = byte & 0x0F;
    
    return Key::Sbox[high][low];
...
void Key::XOR(std::array<uint8_t, Key::WORD_SIZE>* word, std::array<uint8_t, Key::WORD_SIZE> key) {
...
void Key::KeyExpansion() {
    ...
    int Nb = File::BLOCK_SIZE / Key::WORD_SIZE;
    ...
    for(int i = 0; i < nbWords; i+=1) {
        ...
        if (i < Nk) {
            ...
            if(i%Nk == 0) {
                Key::RotWord(&tmp);
                Key::SubWord(&tmp);
                Key::XOR(&tmp, Key::WordRcon( i / Nk - 1));
            }
            if(i%Nk == 4 && Nk == 8) {
                Key::SubWord(&tmp);
            }
            Key::XOR(&tmp, this->RoundKeysWords[i - Nk]);
...
void Key::AddRoundKey(std::array< std::array< uint8_t, File::BLOCK_DIMENSION >, File::BLOCK_DIMENSION>* block, int round) {
    for(int i=round*File::BLOCK_DIMENSION; i < (round+1)*File::BLOCK_DIMENSION; i+=1) {      
        Key::XOR( block->begin()+i, this->RoundKeysWords[i] );
    }
}
